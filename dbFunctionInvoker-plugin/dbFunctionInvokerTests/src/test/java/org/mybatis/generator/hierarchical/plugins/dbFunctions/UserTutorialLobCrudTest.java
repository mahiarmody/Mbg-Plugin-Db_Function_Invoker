...
public class UserTutorialLobCrudTest
{
	...
	{
		UserTutorialWithBLOBs usrTutBlbSelective = new UserTutorialWithBLOBs();
		...
		UserTutorialWithBLOBs usrTutBlbPlain = new UserTutorialWithBLOBs();
	...
	public void test07SelectByExampleWithBlobs()
	{
		...
		List<UserTutorialWithBLOBs> lstUsrTutsWithBLOBs = null;

		SqlSession sqlSession = sqlSessionFactory.openSession();
		try
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			lstUsrTutsWithBLOBs = mapper.selectByExampleWithBLOBs(ex);
		}
		finally
		{
			sqlSession.close();
		...
		Assert.assertEquals("Incorrect number of matching UserTutorial rows returned for PK1", 1, lstUsrTutsWithBLOBs.size());
		Assert.assertNotNull("UserTutorial with PK=1 not found.", lstUsrTutsWithBLOBs.get(0));
		Assert.assertEquals("Incorrect summary for PK1", "This is a large summ...", lstUsrTutsWithBLOBs.get(0).getSummary());
		Assert.assertEquals("Incorrect narrative for PK1", "The narrative goes here", lstUsrTutsWithBLOBs.get(0).getNarrative());
		Assert.assertEquals("Incorrect video type for PK1", "flac", lstUsrTutsWithBLOBs.get(0).getVideoType());
		...
		Assert.assertEquals("Incorrect number of matching UserTutorial rows returned for PK2", 1, lstUsrTutsWithBLOBs.size());
		Assert.assertNotNull("UserTutorial with PK=2 not found.", lstUsrTutsWithBLOBs.get(0));
		Assert.assertEquals("Incorrect summary for PK2", "The summary of 2nd t...", lstUsrTutsWithBLOBs.get(0).getSummary());
		Assert.assertEquals("Incorrect narrative for PK2", "Linux is the way to go!", lstUsrTutsWithBLOBs.get(0).getNarrative());
		Assert.assertNull("Incorrect video type for PK2", lstUsrTutsWithBLOBs.get(0).getVideoType());
		Assert.assertNull("Incorrect video for PK2", lstUsrTutsWithBLOBs.get(0).getVideo());
		...
		Assert.assertEquals("Incorrect number of matching UserTutorial rows returned for PK3", 1, lstUsrTutsWithBLOBs.size());
		Assert.assertNotNull("UserTutorial with PK=3 not found.", lstUsrTutsWithBLOBs.get(0));
		Assert.assertEquals("Incorrect summary for PK3", "Must be at least 20 ...", lstUsrTutsWithBLOBs.get(0).getSummary());
		Assert.assertEquals("Incorrect narrative for PK3", "I'll teach you how to use the db invoker plugin", lstUsrTutsWithBLOBs.get(0).getNarrative());
		Assert.assertEquals("Incorrect video type for PK3", "mp4", lstUsrTutsWithBLOBs.get(0).getVideoType());
		Assert.assertArrayEquals("Incorrect video for PK3", binVideo, lstUsrTutsWithBLOBs.get(0).getVideo());
		...
		Assert.assertTrue("No rows should have matched this select by example criterion, but some did", lstUsrTutsWithBLOBs.size() == 0);
	}
	...
	{
		UserTutorialKey[] userTutKeyPkArr = {new UserTutorialKey(), new UserTutorialKey()};
		userTutKeyPkArr[0].setUserTutorialId(Integer.valueOf(3));
		userTutKeyPkArr[1].setUserTutorialId(Integer.valueOf(4));

		UserTutorialWithBLOBs[] usrTutWithBLOBsArr = new UserTutorialWithBLOBs[2];
		...
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			usrTutWithBLOBsArr[0] = mapper.selectByPrimaryKey(userTutKeyPkArr[0]);
			usrTutWithBLOBsArr[1] = mapper.selectByPrimaryKey(userTutKeyPkArr[1]);
		}
		finally
		{
			sqlSession.close();
		...
		Assert.assertNotNull("UserTutorial with PK=3 not found.", usrTutWithBLOBsArr[0]);
		Assert.assertEquals("Incorrect userTutorialId for PK3", Integer.valueOf(3), usrTutWithBLOBsArr[0].getUserTutorialId());
		Assert.assertEquals("Incorrect userId FK for PK3", Integer.valueOf(1), usrTutWithBLOBsArr[0].getUserId());
		Assert.assertEquals("Incorrect title for PK3", "Third Tutorial", usrTutWithBLOBsArr[0].getTitle());
		Assert.assertEquals("Incorrect summary for PK3", "Must be at least 20 ...", usrTutWithBLOBsArr[0].getSummary());
		Assert.assertEquals("Incorrect narrative for PK3", "I'll teach you how to use the db invoker plugin", usrTutWithBLOBsArr[0].getNarrative());
		Assert.assertEquals("Incorrect video type for PK3", "mp4", usrTutWithBLOBsArr[0].getVideoType());
		Assert.assertArrayEquals("Incorrect video for PK3", binVideo, usrTutWithBLOBsArr[0].getVideo());

		Assert.assertNull("UserTutorial with PK=4 exists, but should not.", usrTutWithBLOBsArr[1]);
	}
	...
	{
		UserTutorialWithBLOBs recNewValues = new UserTutorialWithBLOBs();
	...
	{
		UserTutorialKey usrTutKey = new UserTutorialKey();
		usrTutKey.setUserTutorialId(Integer.valueOf(3));
		...
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			cnt = mapper.deleteByPrimaryKey(usrTutKey);
			sqlSession.commit();
		}
		finally
		{
	...
	{
		UserTutorialKey usrTutKey = new UserTutorialKey();
		usrTutKey.setUserTutorialId(Integer.valueOf(userTutorialId));

		UserTutorialWithBLOBs utb = null;
		...
		{
			UserTutorialMapper mapper = sqlSession.getMapper(UserTutorialMapper.class);
			utb = mapper.selectByPrimaryKey(usrTutKey);
		}
		finally
		{
			sqlSession.close();
