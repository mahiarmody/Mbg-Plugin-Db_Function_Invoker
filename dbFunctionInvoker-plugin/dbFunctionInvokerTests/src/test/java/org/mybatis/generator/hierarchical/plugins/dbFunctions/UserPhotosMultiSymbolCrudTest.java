...
public class UserPhotosMultiSymbolCrudTest
{
	...
	{
		UserPhotosWithBLOBs usrPhotoWithBLOBsSelective = new UserPhotosWithBLOBs();
		usrPhotoWithBLOBsSelective.setUserId(Integer.valueOf(1));
		usrPhotoWithBLOBsSelective.setPhotoTitle("J2EE stack");

		UserPhotosWithBLOBs usrPhotoWithBLOBsPlain = new UserPhotosWithBLOBs();
		usrPhotoWithBLOBsPlain.setUserId(Integer.valueOf(1));
		usrPhotoWithBLOBsPlain.setPhotoTitle("Core Java Layers");
		usrPhotoWithBLOBsPlain.setPhoto(binPhotoData);
		usrPhotoWithBLOBsPlain.setPhotoType("jpg");
		...
		try
		{
			...
			cnt[0] = mapper.insertSelective(usrPhotoWithBLOBsSelective);

			cnt[1] = mapper.insert(usrPhotoWithBLOBsPlain);
		...
		Assert.assertEquals("Number of UserPhotosWithBLOBs records created using insert selective is wrong", 1, cnt[0]);
		Assert.assertEquals("Number of UserPhotosWithBLOBs records created using insert is wrong", 1, cnt[1]);
	}
	...
	public void test10SelectByExampleWithBlobs()
	{
		...
		List<UserPhotosWithBLOBs> lstUsrPhotosWithBLOBs = null;

		SqlSession sqlSession = sqlSessionFactory.openSession();
		try
		{
			UserPhotosMapper mapper = sqlSession.getMapper(UserPhotosMapper.class);
			lstUsrPhotosWithBLOBs = mapper.selectByExampleWithBLOBs(ex);
		}
		finally
		{
			sqlSession.close();
		...
		Assert.assertEquals("Too many rows matching UserPhotosWithBLOBs returned for Core Java Layers", 1, lstUsrPhotosWithBLOBs.size());
		Assert.assertNotNull("UserPhotosWithBLOBs with Core Java Layers not found.", lstUsrPhotosWithBLOBs.get(0));
		Assert.assertTrue("Incorrect binary photo data for Core Java Layers photo", java.util.Arrays.equals(binPhotoData, lstUsrPhotosWithBLOBs.get(0).getPhoto()));
		...
		Assert.assertEquals("Too many rows matching UserPhotosWithBLOBs returned for NULL photo type", 1, lstUsrPhotosWithBLOBs.size());
		Assert.assertNotNull("UserPhotosWithBLOBs with NULL photo type not found.", lstUsrPhotosWithBLOBs.get(0));
		Assert.assertNull("Incorrect binary photo data NULL photo type", lstUsrPhotosWithBLOBs.get(0).getPhoto());
	}
	...
	{
		UserPhotosWithBLOBs recNewValues = new UserPhotosWithBLOBs();
	...
	private boolean isUpdateSuccessful(String paramPhotoTitle, String resultPhotoTitle, String photoType)
	{
		...
		UserPhotosWithBLOBs usrPhotosWithBLOBs = null;

		SqlSession sqlSession = sqlSessionFactory.openSession();
		try
		{
			UserPhotosMapper mapper = sqlSession.getMapper(UserPhotosMapper.class);
			usrPhotosWithBLOBs = mapper.selectByExampleWithBLOBs(ex).get(0);
		}
		finally
		{
			sqlSession.close();
		...
		return resultPhotoTitle.equals(usrPhotosWithBLOBs.getPhotoTitle())
			&& java.util.Arrays.equals(binPhotoData, usrPhotosWithBLOBs.getPhoto())
			&& (photoType == null ? usrPhotosWithBLOBs.getPhotoType() == null : photoType.equals(usrPhotosWithBLOBs.getPhotoType()));
	}
}
